...
# ******************************************************************************** #
# ******************************************************************************** #
# FA_large_grade_top5.py
# 故事看起来在程序E就结束了，但需求总是在变的，这不，新计划就来了
# 神经网络B只需要预测危险区域的单元，但这依然太多了（例如：大型零件危险区域1就有735个），能不能再少一些？
# 我们从“危险单元”中再定义一部分单元，称之为“核心单元”，让神经网络B只预测核心单元（实际表明，大型零件危险区域1核心单元只有125个）
...
# 另外，一共有8所学校，每所学校都依此种方式挑选学生
# 反映到本项目：
# 1所学校就对应1个危险区域，8所学校就是小型零件的8个危险区域
...
# 大型零件67285种载荷组合，就是进行了67285场考试
# 各学生的考试成绩，就是各网格单元的mises应力
# 程序FA干的事，就是列出这67285场考试8所学校的前5名
# **************************************** #
...
# 举例：
# 理论上：
# large_B_crit_top5.csv是67285×41的
# 67285是样本数，41列依次是：样本编号、危险区域1的前5名、……、危险区域8的前5名
# 实际上：
# large_B_crit_top5.csv是n×41的
# n取决于大数据集的文件夹里放了多少个样本
# 更准确地说：
# 41列依次是：样本编号、40个区域顺序编号（8个区域各前5名）
