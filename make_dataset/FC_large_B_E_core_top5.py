...
# 从哪里取数据：<NN_B_data_folder><NN_B_data_E_file>
# 数据放到哪里：<NN_B_data_folder><NN_B_data_E_core_top5_file>
# 本程序的作用：
# 读取上述这些东西，输出1个csv文件
# 注意：
# 本程序只处理大型零件，而且只制作神经网络B应变的数据集
# 如果要完整地制作出训练神经网络B的小数据集，本程序需要运行多次
# 记得每次运行前，都务必检查part_configuration.xml的设置，并且修改第56行的内容
# 尤其是设置好<NN_B_data_E_core_top5_file>，即输出文件名
# ******************************************************************************** #
# ******************************************************************************** #
...
# 对于大型零件，危险区域1的应变的数据集：
# 理论上：
# large_B_E_R1.csv是62234×757的
# 62234是样本数，757列依次是：样本编号、6个载荷、6个应变分量×125个网格单元（危险区域1的）
# 实际上：
# large_B_E_R1.csv是n×757的
...
NN_B_data_E_file = xml_msg.find('NN_B/NN_B_data_E_file').text.strip()
path_save = xml_msg.find('path_save').text.strip()
file_core_sort = xml_msg.find('core/core_sort').text.strip()

# 读取输入文件名
NN_B_data_E_core_top5_file = xml_msg.find('NN_B/NN_B_data_E_core_top5_file').text.strip()

# **************************************** #
# 执行程序
# **************************************** #
path_data = os.path.join(NN_B_data_folder, NN_B_data_E_file)
...
data_ans = np.zeros((len_NN, int(len_crit * 6 + 7)))
data_ans_each = np.zeros(int(len_crit * 6 + 7))

for i in range(len_NN):
    data_ans_each[0:7] = data_NN[i][0:7]
    for k in range(len_crit):
        elem_order = int(data_crit[k][0])
        for j in range(6):
            id_ans = int(k * 6 + j + 7)
            id_elem_order = int(elem_order * 6 + j + 7)
            data_ans_each[id_ans] = data_NN[i][id_elem_order]
    data_ans[i] = data_ans_each

df_ans = pd.DataFrame(data_ans)
path_E_core = os.path.join(NN_B_data_folder, NN_B_data_E_core_top5_file)
df_ans.to_csv(path_E_core, header=False, index=False, encoding='utf-8')
