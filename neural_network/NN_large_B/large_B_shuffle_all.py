...
name_data_R1 = 'large_B_data_S_R1.csv'
name_testset = 'large_B_test_S_all.csv'
name_shuffle = 'large_B_shuffle_id_S_all.csv'
...
for j in range(len_test):
    data_test[j][0:int(num_each_col[0])] = data_NN_R1[shuffle_id[len_test + j]][0:int(num_each_region[0] + 7)]
print(path_data_R1)

for i in range(7):
    name_data = 'large_B_data_S_R' + str(i + 2) + '.csv'
    path_data = path_save + name_data
    print(path_data)
    data_NN = pd.read_csv(path_data, header=None, encoding='utf-8').to_numpy()
    for j in range(len_test):
        data_test[j][int(num_each_col[i]):int(num_each_col[i + 1])] = data_NN[shuffle_id[len_test + j]][
