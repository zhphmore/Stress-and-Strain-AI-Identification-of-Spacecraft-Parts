...
test_df = small_A_selfdataset.SelfDataset(path_testset)
...
ans = np.zeros((test_num, 24))
ans_each = np.zeros(24)
...
with torch.no_grad():
    for step, data in enumerate(test_loader):
        if step < test_num:
            ...
            in_val, out_val, real_val = small_A_selfdataset.revertData(test_X.cpu(), outputs.cpu(), test_Y.cpu())
            ...
            for k in range(6):
                ...
                ans_each[3 * k + 6] = out_val[k]
                ans_each[3 * k + 7] = real_val[k]
                ans_each[3 * k + 8] = abs(mis_each[k] / out_val[k])
            ans[step] = ans_each
