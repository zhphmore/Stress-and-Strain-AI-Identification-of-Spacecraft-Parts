...
name_pth = 'small_B_E_core_top5_R' + str(target_field_id) + '_1.pth'
name_testset = 'small_B_test_E_core_top5_R' + str(target_field_id) + '.csv'
...
test_df = small_B_selfdataset.SelfDataset_E_core(path_testset, target_field_id)
...
loaded_model = small_B_model.NN_E_core(target_field_id)
...
ans = np.zeros((test_num, int(6 * num_this_region)))
ans_pct = np.zeros((test_num, int(6 * num_this_region)))
ans_each = np.zeros(int(6 * num_this_region))
ans_each_pct = np.zeros(int(6 * num_this_region))
max_mis = 0
max_pct = 0
ave_mis = 0
ave_pct = 0
...
with torch.no_grad():
    for step, data in enumerate(test_loader):
        if step < test_num:
            ...
            for j in range(int(6 * num_this_region)):
...
path_4 = path_save + 'small_B_ans_E_core_top5_R' + str(target_field_id) + '.csv'
