...
name_pth = 'small_B_S_core_top5_R' + str(target_field_id) + '_1.pth'
name_trainset = 'small_B_train_S_core_top5_R' + str(target_field_id) + '.csv'
name_testset = 'small_B_test_S_core_top5_R' + str(target_field_id) + '.csv'
name_refer = 'small_B_S_core_top5_R' + str(target_field_id) + '_1.pth'
...
train_df = small_B_selfdataset.SelfDataset_S_core(path_trainset, target_field_id)
train_num = len(train_df)
train_loader = DataLoader(train_df, batch_size=batch_size, shuffle=True)
# 读取测试集
path_testset = path_save + name_testset
test_df = small_B_selfdataset.SelfDataset_S_core(path_testset, target_field_id)
test_num = len(test_df)
test_loader = DataLoader(test_df, batch_size=batch_size, shuffle=True)

# **********模型训练设置**********
# 模型
model = small_B_model.NN_S_core(target_field_id)
# 设备
...
for i in range(epochs):
    ...
    for step, data in enumerate(train_bar):
        ...
        loss = criterion_1(outputs, train_Y1)
        loss.backward()
        optimizer.step()
    ...
    with torch.no_grad():
        ...
        for step, data in enumerate(test_bar):
            ...
            if torch.cuda.is_available():
                test_X = test_X.cuda()
                test_Y1 = test_Y1.cuda()

            outputs = model(test_X)
            test_loss = criterion_2(outputs, test_Y1)
