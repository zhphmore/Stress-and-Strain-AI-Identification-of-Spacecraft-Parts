...
name_pth = 'small_B_S_R' + str(target_field_id) + '_1.pth'
...
loaded_model = small_B_model.NN_S(target_field_id)
...
with torch.no_grad():
    for step, data in enumerate(test_loader):
        if step < test_num:
            ...
            ans_each[0] = outputs.max() * 1000
            # ans_each[1] = test_Y1.max() * 500
            id_arg = outputs.argmax()
            ans_each[1] = test_Y1[0][id_arg] * 1000
...
path_4 = path_save + 'large_B_ans_SR' + str(target_field_id) + '.csv'
